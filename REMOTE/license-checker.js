 ...
 *
 */
class LicenseChecker extends EventTarget {
  ...
  getDialogOkCallback() {
    const self = this;
    return () => {
      const ev = self.createEvent(LicenseChecker.EventType.AC_SETTINGS_APPLY);
   ...
   */
  getDialogContent() {
    return $('<h2>Anychart License Text Title</h2>' +
   ...
   */
  getDialog() {
    return new DialogBox(this.getDialogContent(), "Anychart License Dialog Title", "Ok", null, this.getDialogOkCallback());
  }
  ...
  getSettingsButtonClickCallback() {
    const self = this;
    return (e) => {
      const ev = self.createEvent(LicenseChecker.EventType.AC_DIALOG_SHOW, {originalEvent: e});
   ...
   */
  wrapFreeboardTools() {
    const e = this.createEvent(LicenseChecker.EventType.AC_WRAP_TOOLS);
